    ...
    public partial class Cashier : Form
    {
        ...
        public void LoadCart()
        {
            try
            {
                ...
                dgvCash.Rows.Clear();
                cn.Open();
                cm = new SqlCommand("SELECT c.id, c.pcode, p.pdesc, c.price, c.qty, c.dis, c.total FROM tbCart AS c INNER JOIN tbProduct AS p ON c.pcode=p.pcode WHERE c.transno LIKE @transno and c.status LIKE 'Pending'", cn);
                cm.Parameters.AddWithValue("@transno", lblTranNo.Text);
                dr = cm.ExecuteReader();
                while (dr.Read())
                {
                    ...
                    discount += Convert.ToDouble(dr["dis"].ToString());
                    dgvCash.Rows.Add(i, dr["id"].ToString(), dr["pcode"].ToString(), dr["pdesc"].ToString(), dr["price"].ToString(), dr["qty"].ToString(), dr["dis"].ToString(), double.Parse(dr["total"].ToString()).ToString("#,##0.00"));//
                    hascart = true;
                }
                dr.Close();
                cn.Close();
        ...
        public void GetCartTotal()
        {
            ...
            double cgst = sales * 0.06; 
            double sgst = sales * 0.06; 
            double totalGST = cgst + sgst; 
            double taxableAmount = sales; 


            lblVat.Text = cgst.ToString("#,##0.00");
            lblVatable.Text = sgst.ToString("#,##0.00");
