...
namespace POSales
{
    public partial class DailySale : Form
    {
        ...
        public void LoadSold()
        {
            ...
            //{
            //    cm = new SqlCommand("select c.id, c.transno, c.pcode, p.pdesc, c.price, c.qty, c.dis, c.total from tbCart as c inner join tbProduct as p on c.pcode = p.pcode where status like 'Sold' and sdate between '" + dtFrom.Value + "' and '" + dtTo.Value + "'", cn);
            //}
            //else
            //{
            //    cm = new SqlCommand("select c.id, c.transno, c.pcode, p.pdesc, c.price, c.qty, c.dis, c.total from tbCart as c inner join tbProduct as p on c.pcode = p.pcode where status like 'Sold' and sdate between '" + dtFrom.Value + "' and '" + dtTo.Value + "' and cashier like '" + cboCashier.Text + "'", cn);
            //}
            //dr = cm.ExecuteReader();
            //while(dr.Read())
            //{
            ...
            //    dgvSold.Rows.Add(i, dr["id"].ToString(), dr["transno"].ToString(), dr["pcode"].ToString(), dr["pdesc"].ToString(), dr["price"].ToString(), dr["qty"].ToString(), dr["dis"].ToString(), dr["total"].ToString());
            //}
            //dr.Close();
            //cn.Close();
            ...
            string query = "SELECT c.id, c.transno, c.pcode, p.pdesc, c.price, c.qty, c.dis, c.total " +
                           "FROM tbCart AS c " +
            ...
            cm.Parameters.AddWithValue("@dtFrom", dtFrom.Value.ToString("yyyy-MM-dd"));
            cm.Parameters.AddWithValue("@dtTo", dtTo.Value.ToString("yyyy-MM-dd"));
            ...
            while (dr.Read())
            {
                i++;
                total += double.Parse(dr["total"].ToString());
                dgvSold.Rows.Add(i, dr["id"].ToString(), dr["transno"].ToString(), dr["pcode"].ToString(), dr["pdesc"].ToString(),
                                 dr["price"].ToString(), dr["qty"].ToString(), dr["dis"].ToString(), dr["total"].ToString());
            }
